...
class ExactInference(InferenceModule):
    ...
    """
    # Pacman Believes that the ghost could be anywhere
    def initializeUniformly(self, gameState):
    ...
    def observe(self, observation, gameState):
        ...
        locationDict = util.Counter()

        # Ghost is captured so noisy Distsance is None
        if noisyDistance == None:
            locationDict[self.getJailPosition()] = 1.0
        else:
            for location in self.legalPositions:
                distance = util.manhattanDistance(location, pacmanPosition)
                locationDict[location] = emissionModel[distance] * self.beliefs[location]
        locationDict.normalize()
        self.beliefs = locationDict
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        if noisyDistance == None:
            self.listParticle = [self.getJailPosition()] * self.numParticles
        else:
            updatedDist = self.getBeliefDistribution()
            for particle in updatedDist:
                dist = util.manhattanDistance(pacmanPosition,particle)
                updatedDist[particle] *= emissionModel[dist]
            if updatedDist.totalCount() == 0.0:
                self.initializeUniformly(gameState)
            else:
                updatedDist.normalize()
                self.listParticle = [util.sample(updatedDist) for i in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        """
        newParticles = []
        for particle in self.listParticle:
                updatedDist = self.getPositionDistribution(self.setGhostPosition(gameState, particle))
                newParticles.append(util.sample(updatedDist))
        self.listParticle = newParticles
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] != None:
                    probability *= emissionModels[i][util.manhattanDistance(pacmanPosition, particle[i])]
            resampledDict[particle] += probability

        helper = True
        for i in resampledDict.values():
            if i > 0:
                helper = False
                break
        listOfSamples = []
        if helper:
            ...
            for particle in self.particles:
                for i in range(self.numGhosts):
                    if noisyDistances[i] == None:
                        particle = self.getParticleWithGhostInJail(particle, i)
                listOfSamples.append(particle)
            self.particles = listOfSamples
            return
        for i in range(0, self.numParticles):
            listOfSamples.append(util.sample(resampledDict))
        self.particles = listOfSamples
